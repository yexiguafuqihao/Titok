...
class VQGANTrainer(nn.Module):
    def __init__(
        ...
        self.train_dl = DataLoader(self.train_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory, collate_fn=collate_fn)
        self.valid_dl = DataLoader(self.valid_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory, collate_fn=collate_fn)
        ...
        if (model.params.ckpt_path is not None) & osp.exists(model.params.ckpt_path):
            self.steps = int(osp.basename(model.params.ckpt_path).split('.')[0].split('_')[-1])
    ...
    def calculate_loss_gradient_penalty(self, rec, img, lambda_term=10):
        ...
        interpolated = self.calculate_gradient_penalty(img, rec,)
                ...
                for batch in train_dl:
                    ...
                    with self.accelerator.accumulate(self.discr):
                        with self.accelerator.autocast():
                            ...
                            d_loss, grad_penalty = self.calculate_loss_gradient_penalty(rec, img)
                    ...
                    with self.accelerator.accumulate(self.vqvae):
                        with self.accelerator.autocast():
                            ...
                            # combine
                            loss = sum([v for _, v in codebook_loss.items()]) + rec_loss + per_loss + self.d_weight * g_loss
