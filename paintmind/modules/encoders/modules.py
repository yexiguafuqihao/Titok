...
from .pretrained_enc_arch import SelfSupervisedCondtionEmbedder, AbstractEmbModel, PLACEHOLDEREmbedder
...
class GeneralConditioner(nn.Module):
    ...
    def __init__(self, emb_models: Union[List, ListConfig]):
        ...
        for n, embconfig in enumerate(emb_models):
            ...
            ) | isinstance(embedder, SelfSupervisedCondtionEmbedder), f"embedder model {embedder.__class__.__name__} has to inherit from AbstractEmbModel"
            ...
            if "input_key" in embconfig:
                ...
                if isinstance(embedder, PLACEHOLDEREmbedder):
                    pass
                else:
                    raise KeyError(
        ...
        self.input_names = [embedder.input_key for embedder in embedders if embedder is not None]
    ...
    def forward(
        ...
        self_emb, useless = None, []
        for i, embedder in enumerate(self.embedders):
            
            if isinstance(embedder, PLACEHOLDEREmbedder) | isinstance(embedder, FrozenOpenCLIPImagePredictionEmbedder) | (embedder is None):
                continue
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    if embedder.input_key in ['motion_bucket_id', 'fps_id', 'cond_aug']:
                        value = batch[embedder.input_key]
            ...
            if not isinstance(emb_out, (list, tuple)):
                emb_out = [emb_out]
            if not isinstance(embedder, SelfSupervisedCondtionEmbedder):
                for emb in emb_out:
                ...
                if rep.size(-1) < 1024:
                    bs, t, n, c = rep.shape
                    zeros = rep.new_zeros((bs, t, n, 1024 - c))
                    output['rep'] = torch.cat((rep, zeros), dim=-1)
        return output
