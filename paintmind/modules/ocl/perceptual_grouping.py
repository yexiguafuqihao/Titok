...
from ..ocl.typing import (FeatureExtractorOutput, ConditioningOutput,
                          EmptyIndicator,PerceptualGroupingOutput,
...
class SlotAttention(nn.Module):
    ...
    def forward(
        self, inputs: torch.Tensor, slots: torch.Tensor, masks: Optional[torch.Tensor] = None
    ):
        b, n, d = inputs.shape
...
def build_mlp(in_channels, middle_channel, out_channels, num_layers):

    return MLP(in_channels, middle_channel, out_channels, num_layers)
...
class SlotAttentionGrouping(nn.Module):
    ...
    def __init__(
        ...
        ff_mlp = build_mlp(object_dim, object_dim, object_dim, 2)
    ...
    def forward(self, feature, conditioning, slot_mask = None):
        ...
        if self.positional_embedding:
            feature = self.positional_embedding(feature, feature.positions)
        else:
            feature = feature
...
class StickBreakingGrouping(nn.Module):
    ...
    def forward(
        ...
        proj_features = F.normalize(self.in_proj(features), dim=-1)
...
class KMeansGrouping(nn.Module):
    ...
    def forward(
        ...
        for feat in batch_features:
            ...
            cluster_ids.append(clustering.fit_predict(feat).astype(np.int64))
            cluster_centers.append(clustering.cluster_centers_)

        cluster_ids = torch.from_numpy(np.stack(cluster_ids))
        cluster_centers = torch.from_numpy(np.stack(cluster_centers))
